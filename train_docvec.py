...
# unlabelled = dataset + "2015_Nepal_Earthquake_unlabelled.tsv"

dev = dataset + "2015_Nepal_Earthquake_dev.tsv"
dev_tweets = pd.read_csv(dev, sep="\t", encoding="latin")
...
for tweet_collection in [train_tweets, dev_tweets]:
	for index, row in tweet_collection.iterrows():
...
model = Doc2Vec(seed=0,workers = multiprocessing.cpu_count(), window=5,  callbacks=[epoch_logger],min_count=1, vector_size=300, epoch=10)
